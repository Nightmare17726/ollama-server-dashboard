...
try:
    with Live(layout, refresh_per_second=2, screen=True) as live:
        while True:
            ...
            layout['ram'].update(Panel(get_speedometer(mem.percent, 'orange1', f'RAM {mem.percent:.0f}% | {mem_used_gb:.1f}/{mem_total_gb:.1f} GB'), title='MEMORY', border_style='orange1'))
            ...
            ref_table.add_row('200', 'OK: Request processed and token stream successful')
            ref_table.add_row('400', 'Bad Request: Malformed JSON or context boundary error')
            ref_table.add_row('404', 'Not Found: Target model tag is missing or not pulled')
            ref_table.add_row('500', 'Server Error: VRAM context allocation failure or crash')
            ...
            ana_grid.add_row(
                Group(
                    Text('\nAPI TRAFFIC REFERENCE', style='bold orange1'),
                    ref_table
                ),
                ''
            )
            ...
            layout['logs'].update(Panel(l_t, title='LIVE API TRAFFIC', border_style='orange1'))
